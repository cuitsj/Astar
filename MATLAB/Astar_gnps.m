...
for i=1:row+1
    for j=1:8:column+1
        fwrite(scom,(map_array(i,j)*1+map_array(i,j+1)*2+map_array(i,j+2)*4+map_array(i,j+3)*8+map_array(i,j+4)*16+map_array(i,j+5)*32+map_array(i,j+6)*64+map_array(i,j+7)*128)); % 以二进制形式向串口对象写入数据A
