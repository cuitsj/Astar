...
//closelist
(*ram_style="block"*) reg [7:0] closelist_parenty[CLOSE_ROW-1:0];//父节点y
(*ram_style="block"*) reg [7:0] closelist_parentx[CLOSE_ROW-1:0];//父节点x
(*ram_style="block"*) reg [7:0] closelist_childy[CLOSE_ROW-1:0];//父节点y
(*ram_style="block"*) reg [7:0] closelist_childx[CLOSE_ROW-1:0];//父节点x
...
//openlist
(*ram_style="block"*) reg [7:0] openlist_parenty[OPEN_ROW-1:0];//父节点y
(*ram_style="block"*) reg [7:0] openlist_parentx[OPEN_ROW-1:0];//父节点x
(*ram_style="block"*) reg [7:0] openlist_childy[OPEN_ROW-1:0];//父节点y
(*ram_style="block"*) reg [7:0] openlist_childx[OPEN_ROW-1:0];//父节点x
    ...
    if (!rst_n) begin
        ...
        case (current_state)
            ...
            S16:begin
                ...
                else if (e31 == 2) current_state <= S20;//规划结束，没有找到最短路径
                else if (e31 == 3) current_state <= S17;//算法结束，并且找到最短路径
